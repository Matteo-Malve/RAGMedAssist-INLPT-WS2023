...
Since fine-tuning takes a lot of time, this script needs to be executed on a GPU!
"""
...
# # Save 10 sentences randomly for evaluation
# eval_sents = random.sample(sentences, 10)
# sentences = [sentence for sentence in sentences if sentence not in eval_sents]    #delete eval sents from list

# Add noise to training data via DenoisingAutoEncoderDataset 
train_data = DenoisingAutoEncoderDataset(sentences)            #it returns InputExamples in the format: texts=[noise_fn(sentence), sentence] 
...
# # Test reconstructing capabilities
# reconstructed_sents = []
# for sent in eval_sents:
#     embedding = model.encode(sent)
#     reconstructed_sent = model.decode(embedding)
#     reconstructed_sents.append(reconstructed_sent)

# for original, reconstructed in zip(eval_sents, reconstructed_sents):
