...
As vector databases, we implemented both Pinecone and FAISS, which serve as the backbone for our efficient information retrieval process. We generated embeddings using `thenlper_gte-base` for semantic search. We set the search type of the retrievers generated by these vectorstores to `similarity_score_threshold` because our RAG system focuses on medical data, and we aim to avoid retrieving documents for unrelated queries. This approach prevents our chatbot from hallucinating and ensures it only replies with `I don't know` when no documents were retrieved. By default, we used FAISS for our dense vectors, which can be changed in `cfg.yaml`.

### Pinecone

Pinecone is a managed, scalable vector database designed for building and deploying large-scale vector search applications in the cloud. Its as-a-service model offers ease of use, scalability, and reliability, making it an ideal choice for projects that require high availability and minimal operational overhead. Pinecone's architecture is optimized for fast vector similarity searches, enabling real-time query responses even within vast datasets. This capability is particularly beneficial for applications that demand the immediate retrieval of highly relevant results based on vector similarity.

### FAISS

FAISS, developed by Facebook AI Research, is an open-source library for efficient similarity search and clustering of dense vectors. It contains algorithms that search in sets of vectors of any size, up to those that possibly do not fit in RAM. FAISS is particularly noted for its performance optimization and resource efficiency, making it suitable for local deployment where direct control over computational resources is required. Its extensive collection of pre-configured indexing strategies allows for fine-tuning based on the specific balance of speed and accuracy needed for the task.

### Comparison Between FAISS and Pinecone

For this comparison, we created two retrievers with the same configurations: one uses FAISS and the other Pinecone as the vector store. The evaluation was conducted over a set of 167 queries, comparing the performance based on the following metrics:
...
To sum up, FAISS satisfied all our needs and was faster than Pinecone. The only disadvantage was that we need to store our FAISS indices locally, which corresponds to almost 200MB. Pinecone is a commercial vector store and will be actively developed. It offers more functions than FAISS, such as ensemble retriever or metadata filtering, but these extra functions can only be accessed with a paid account.

---

## <a name="vectorstore"></a>4.2 📥 Vector Database
As a vector database, we implemented both Pinecone and FAISS, which serve as the backbone for our efficient information retrieval process. 
We generated the embeddings using `thenlper_gte-base` for the semantic search.
We set the search type of the retrievers generated by these vectors `similarity_score_threshold`, because our RAG system concentrates on medical data and we don't want to retrieve any documents for the unrelated queries, so our chatbot won't get hallucinated and just reply with `I don't know`.
We used FAISS by default for our dense vectors which can be change in `cfg.yaml`.


### Pinecone
Pinecone is a managed, scalable vector database designed for building and deploying large-scale vector search applications in the cloud. Its as-a-service model offers ease of use, scalability, and reliability, making it an ideal choice for projects requiring high availability and minimal operational overhead. Pinecone's architecture is optimized for fast vector similarity search, enabling real-time query responses even within vast datasets. This capability is particularly beneficial for applications that demand immediate retrieval of highly relevant results based on vector similarity.

### FAISS
FAISS, developed by Facebook AI Research, is an open-source library for efficient similarity search and clustering of dense vectors. It contains algorithms that search in sets of vectors of any size, up to ones that possibly do not fit in RAM. FAISS is particularly noted for its performance optimization and resource efficiency, making it suitable for local deployment where direct control over computational resources is required. Its extensive collection of pre-configured indexing strategies allows for fine-tuning based on the specific balance of speed and accuracy needed for the task.


### Comparison Between FAISS and Pinecone
For this comparison, we created to retrievers with the same configurations, one uses FAISS and the other Pinecone as Vectorstore.
The evaluation was conducted over a set of 167 queries, comparing the performance based on the following metrics:
Firstly, we compared the speed of both retrievers generated by these vectorstore. 
It turned out that FAISS retrieves the `topk` context for all 176 instances only in only 4 seconds while it takes for the Pinecone over 40 seconds
<p align="middle">
  <img src="./evaluation/compare_retrievers/images/total_execution_time_plot.png" width="400"/>
</p>
The comparison was happened with a stable internet connection. Because FAISS is a local vectorstore, it is way faster than pinecone.

For each query, we have also the context in our dataset, and the query and its answer were generated based on that context.
Secondly, we compared the percentage of , if the correct context is amongs the retrieved docs for different `topk`.
As expected, the result was almost identical, because both vectorstore uses the same embeddings.
<p align="middle">
  <img src="./evaluation/compare_retrievers/images/success_percentage_plot.png" width="400"/>
</p>

To sum up, FAISS satisfied all our needs and more faster than Pinecone. Teh only disanvantage was, that we need to store our FAISS indices locally, which corresponst to almost 200MB. Pinecone is a commercial vectorstore in it will be developed actively. It has more functions than FAISS such as ensemble retriever, or metadata filtering, but the extra functions can only be accessed with the paid account.
